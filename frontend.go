...
func Frontend(ctx context.Context, c client.Client) (*client.Result, error) {
	...
	def, err := context.Marshal(llb.WithCustomName("Autodetecting from context"))
	if err != nil {
		return nil, err
	}
	...
	if detect == "" {
		return nil, fmt.Errorf("failed to autodetect")
	}

	st := inputs[detect]
	def, err = st.Marshal(llb.WithCustomNamef("Detected %s project", detect))
	if err != nil {
		return nil, err
	}
